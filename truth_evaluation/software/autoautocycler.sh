...
for reads in "${READ_FILES[@]}"; do
	...
	# canu
	mkdir -p ${assemblies}/canu_0${i}/assembly
	/hps/nobackup/iqbal/dander/amira_paper_latest/truth_evaluation_thesis_panRG/software/canu-2.3/bin/canu -p assembly -d ${assemblies}/canu_0${i}/assembly useGrid=false genomeSize=5m -nanopore ${subsampled_reads}/sample_0${i}.fastq
	mv ${assemblies}/canu_0${i}/assembly/assembly.contigs.fasta ${assemblies}/canu_0${i}.fasta
        rm -rf ${assemblies}/canu_0${i}
	# flye
	flye -g 5M -o ${assemblies}/flye_0${i} -t $THREADS --nano-raw ${subsampled_reads}/sample_0${i}.fastq
	mv ${assemblies}/flye_0${i}/assembly.fasta ${assemblies}/flye_0${i}.fasta
	rm -rf ${assemblies}/flye_0${i}
        # raven
	mkdir -p ${assemblies}/raven_0${i}
        raven -t $THREADS ${subsampled_reads}/sample_0${i}.fastq > ${assemblies}/raven_0${i}/assembly.fasta
	mv ${assemblies}/raven_0${i}/assembly.fasta ${assemblies}/raven_0${i}.fasta
	rm -rf ${assemblies}/raven_0${i}
    done
    rm -rf $subsampled_reads  # Remove reads directory to save disk space

    # Cluster and compress unitig graph
    autocycler compress -i $assemblies -a $autocycler_out -t $THREADS --kmer $KMER --max_contigs 100
    autocycler cluster -a $autocycler_out --max_contigs 100
    ...
    pypolca run --careful -a ${autocycler_out}/consensus_assembly.fasta -1 $ILL1 -2 $ILL2 -t $THREADS -o ${autocycler_out}/pypolca -f
