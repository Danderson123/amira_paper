...
amira_outputs = glob.glob("evaluation_results/Amira_output.v0.11.0/*/amira_results.tsv")


def prev_apply_rules(gene):
    gene = gene.replace("'", "")
    if "blaCTX-M" in gene:
        gene = "blaCTX-M"
    if "blaNDM" in gene:
...
for a in tqdm(amira_outputs):
    ...
    if not all(row["Amira allele"] in validation_coverages for index, row in amira_content.iterrows()):
        continue
    amira_counts = {}
...
df = df.sort_values(by="Max_Freq", ascending=True)

plt.rcParams.update({'font.family': 'sans-serif', 'font.size': 24})
...
for idx, row in df.iterrows():
    ...
    plt.plot([amira_freq, amrfp_freq], [x, x], color='gray', zorder=1)
    plt.scatter(amira_freq, x, label="Amira", marker="o", color=palette[0], zorder=2,  edgecolor='black', s=200)
    plt.scatter(amrfp_freq, x, label="AMRFP Flye", marker="X",  edgecolor='black', color=palette[1], zorder=2, s=300)
ax.set_yticks(range(len(df)))
ax.set_yticklabels(df["Gene"], fontstyle='italic', fontsize=24)
ax.set_xlabel("AMR gene-presence frequency", fontsize=24)
ax.set_xlim([0.00001, 1.1])
ax.set_xticks([0.00001, 0.0001, 0.001, 0.01, 0.1, 1])
ax.set_xticklabels(["0", "0.0001", "0.001", "0.01", "0.1", "1"], fontsize=24)
